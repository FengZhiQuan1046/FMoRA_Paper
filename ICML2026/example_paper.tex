...
% 現在我有一個lora模組UV。 y= (W + IFFT(M_t \odot FFT(UV) + B))x，M_t由0和1組成。第一階段先用八個任務訓練multitask learning訓練UV，每個任務的M_t完全不重合各佔1/8，加起來是一個全是1的矩陣，訓練好以後只保留其各自的M_t \odot FFT(UV)的全部加起來的最終矩陣結果，且在下遊任務不訓練這個稠密。然後下遊任務用特定的任務資料集訓練B，B是一個高度離散的矩陣，只訓練對output最具顯著性的1%頻率。幫我用客觀嚴謹的推導過程分析其梯度，說明其訓練過程是否會有更加容易或者不容易。訓練的\Delta W的基底方向是否可以接近full finetune。
